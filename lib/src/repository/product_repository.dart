...
Future<Stream<Favorite>> getFavorites() async {
  ...
  final String _apiToken = 'api_token=${_user.apiToken}&';
  final String url = '${GlobalConfiguration().getValue('api_base_url')}favorites?${_apiToken}with=product;user;extras&search=user_id:${_user.id}&searchFields=user_id:=';
...
Future<Stream<Product>> getProductsOfMarket(String marketId) async {
  final String url = '${GlobalConfiguration().getValue('api_base_url')}products?with=market&search=market.id:$marketId&searchFields=market.id:=';
...
Future<Stream<Product>> getFeaturedProductsOfMarket(String marketId) async {
  ...
  final String url = '${GlobalConfiguration().getValue('api_base_url')}products?with=market&search=market_id:$marketId&searchFields=market_id:=';
